...
state_query = """
    select state from aqi_project_db.CONSUMPTION_SCH.AGG_CITY_FACT_HOUR_LEVEL 
    group by state 
    order by 1 desc
"""
...
if (state_option is not None and len(state_option) > 1):
    ...
    city_query = f"""
    select city from aqi_project_db.CONSUMPTION_SCH.AGG_CITY_FACT_HOUR_LEVEL 
    where 
...
if (city_option is not None and len(city_option) > 1):
    date_query = f"""
        ...
        from 
        aqi_project_db.CONSUMPTION_SCH.AGG_CITY_FACT_HOUR_LEVEL 
        where 
