...
state_query = """
    select state from aqi_project_db.CONSUMPTION_SCH.LOCATION_DIM 
    group by state 
    order by 1
"""
...
if (state_option is not None and len(state_option) > 1):
    city_query = f"""
    select city from aqi_project_db.consumption_sch.location_dim 
    where 
...
if (city_option is not None and len(city_option) > 1):
    station_query = f"""
    select station from aqi_project_db.consumption_sch.location_dim 
        where 
...
if (station_option is not None and len(station_option) > 1):
    date_query = f"""
    select date(measurement_time) as measurement_date from aqi_project_db.consumption_sch.date_dim
        group by 1 
        order by 1 desc
    """
...
if (date_option is not None):
    ...
    select 
        ...
        AQI
    from 
        aqi_project_db.consumption_sch.air_quality_fact f 
        join 
        aqi_project_db.consumption_sch.date_dim d on d.date_pk  = f.date_fk and date(measurement_time) = '{date_option}'
        join 
        aqi_project_db.consumption_sch.location_dim l on l.location_pk  = f.location_fk and 
