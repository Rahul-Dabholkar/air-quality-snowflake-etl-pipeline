...
-- currently the data is saved as city level hour lwevwel
-- nowwe have the fact and the aggregated tables now we craeate 2 tabnles for 
-- visualisation on streamlit
    -- 1q. city level day level AQI data
    -- 2. city level hour level AQI data

-- lets first chekc data 
select 
    d.measurement_time,
    l.state,
    l.city,
...
-- grouping on d.measurement_time which is hour to get city level hour data
...
select 
    *,
    -- creating city level facts (all cols  + prominent pullutant + calculateed AQI )
...
-- now wwe create day level data with this hour levl table. ez
select 
    * 
